...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if ( ( (mode & O_DIRECTORY )!=0 && (destInode.type == DIRECTORY_TYPE))   ||  ((mode & O_DIRECTORY)==0 &&destInode.type == REGULAR_TYPE))//not finished
		{
			putString("pass mode detect\n");
		}
		else
		{
			putString("mode error\n");
			sf->eax = -1;
			return;
		}
		...
		{
			if(file[i].inodeOffset == destInodeOffset)flag_open = 1; 
		}
		for( int i = 0; i < MAX_DEV_NUM; i++)
		{
			if(dev[i].inodeOffset == destInodeOffset)flag_open = 1; 
		}
		if(flag_open)
		{
			putString("already open\n");
			sf->eax = -1;
			return;
		}
		putString("111error\n");
		...
		if ((sf->edx >> 3) % 2 == 0) { 
			...
			int number = stringLen(str);
			stringCpy(str,temp,number);
			if(temp[number-1] == '/')
			{
				temp[number-1] = 0;
			}
			int res = stringChrR(temp,'/',&size);
			if(res == 0 && size ==0)
			{
				FatherPath[0] = '/';
				FatherPath[1] = 0;
			...
			{
				stringCpy(temp,FatherPath,size);
			}
			ret = readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, FatherPath);
			if(ret < 0)
			{
			...
			//
			ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, temp+size+1 , DIRECTORY_TYPE);
		}
...
void syscallWrite(struct StackFrame *sf) {
	...
	if( sf->ecx >= MAX_DEV_NUM + MAX_FILE_NUM || file[sf->ecx - MAX_DEV_NUM].state != 1 )
	{
		sf->eax = -1;
		return;	
	}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	// 
	int first_block_remain = sBlock.blockSize - remainder;
	if(sz <= first_block_remain)
	{
	...
	{
		sz = sz - first_block_remain;
		int last_bolck_remain = sz % sBlock.blockSize;
		int block_number = sz / sBlock.blockSize;

		readBlock(&sBlock,&inode,quotient,buffer);
		MemCpy(str,&buffer[remainder],first_block_remain);
		cur += first_block_remain;
		writeBlock(&sBlock,&inode,quotient,buffer);
		...
		for( int j = 0; j < block_number; j++)
		{
			quotient++;
			readBlock(&sBlock,&inode,quotient,buffer);
			MemCpy(str+cur,buffer,sBlock.blockSize);
			writeBlock(&sBlock,&inode,quotient,buffer);
			cur += sBlock.blockSize;
		}
		quotient++;
		readBlock(&sBlock,&inode,quotient,buffer);
		MemCpy(str+cur,buffer,last_bolck_remain);
		writeBlock(&sBlock,&inode,quotient,buffer);
		cur += last_bolck_remain;
	}
...
void syscallReadFile(struct StackFrame *sf) {
	...
	else
	{
		...
		readBlock(&sBlock,&inode,quotient,buffer);
		MemCpy(&buffer[remainder],str,first_block_remain);
		cur += first_block_remain;
		...
		for( int j = 0; j < block_number; j++)
		{
			quotient++;
			readBlock(&sBlock,&inode,quotient,buffer);
			MemCpy(buffer,str+cur,sBlock.blockSize);
			cur += sBlock.blockSize;
		}
		quotient++;
		readBlock(&sBlock,&inode,quotient,buffer);
		MemCpy(buffer,str+cur,last_bolck_remain);
		cur += last_bolck_remain;
	}
...
void syscallLseek(struct StackFrame *sf) {
	...
	if(ofs<0||ofs>=inode.size){
		sf->eax=-1;//unsuccess return
		return;
	}
	file[FCBindex].offset = ofs;
	sf->eax=0;//safe return
	return;
}

void syscallClose(struct StackFrame *sf) {
	putString("you are in close\n");
	int i = (int)sf->ecx;
...
void syscallRemove(struct StackFrame *sf) 
{
	...
	{ // file exist
		...
		if (destInode.type == REGULAR_TYPE) {
			...
			//路径是个目录
			int res = stringChrR(str,'/',&size);
			if( size == stringLen(str)-1 || res < 0)
			{
				sf->eax = -1;
				return;
			}
			...
			//find the father
			char FatherPath[NAME_LENGTH];
			stringCpy(str,FatherPath,size+1);//end with '/'
			ret = readInode(&sBlock, gDesc,&fatherInode, &fatherInodeOffset, FatherPath);
			if (ret < 0)
			{
				sf->eax = -1;
			...
			char temp[NAME_LENGTH];
			char FatherPath[NAME_LENGTH];
			stringCpy(str,temp,stringLen(str));//copy the path for following process
			int number = stringLen(temp);
			if(temp[number-1] == '/') 
			{
				temp[number-1] = 0;
			}//replace '/' to 0
			...
			stringCpy(temp,FatherPath,size+1);//copy the father with '/' end
			ret = readInode(&sBlock, gDesc,&fatherInode, &fatherInodeOffset, FatherPath);
			if (ret < 0)
			{
				sf->eax = -1;
