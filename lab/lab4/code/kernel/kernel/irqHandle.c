...
void syscallReadStdIn(struct StackFrame *sf) {
	...
	if(dev[STD_IN].value==-1)//means threre was one process do the read, not finished yet.
	{
	...
	else if(dev[STD_IN].value==0)
	{
		...
		//modified the pcb and rise the timerhandle
	...
	{
		dev[STD_IN].value -= 1;
		dev[STD_IN].pcb.next = &(dev[STD_IN].pcb);
		dev[STD_IN].pcb.prev = &(dev[STD_IN].pcb);
	}
		//when woken, do the stdin work.
		int sel = sf->ds;
		char *str = (char *)sf->edx;
		int size = sf->ebx;
		int i = 0;
...
void syscallSemInit(struct StackFrame *sf) {
	...
	int index = 0; 
	// first modified 5/6 19:33
	for(; index < MAX_SEM_NUM; ++index)
	{
		if (sem[index].state == 0)break; // find a free sem
	} 
	if( index == MAX_SEM_NUM )//not find
	{
		sf->eax = -1;//pcb[current].regs.eax = -1;
	}
	else
	{
		sem[index].state = 1;
		sem[index].value = init_value;
...
void syscallSemWait(struct StackFrame *sf) {
	...
	{
		sem[index].value -= 1;
		if(sem[index].value < 0)//we have someone waiting
		{
