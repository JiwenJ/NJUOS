...
pid_t fork() {
	/*TODO:call syscall*/
	int temp;//definitely return a number(pid or -1)
	temp = syscall(SYS_FORK,0,0,0,0,0);
	return temp;
}

int exec(uint32_t sec_start, uint32_t sec_num){
	/*TODO:call syscall*/
	int temp = syscall(SYS_EXEC,sec_start,sec_num,0,0,0); // return -1(fail) or 0(success)
	return temp;
}

int sleep(uint32_t time) {
	/*TODO:call syscall*/
	syscall(SYS_SLEEP,time,0,0,0,0);// return value has no meaning
	return 0;
}

int exit() {
	/*TODO:call syscall*/
	syscall(SYS_EXIT,0,0,0,0,0); // the same meaning as above
	return 0;
}
